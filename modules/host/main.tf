  ...
  provisioner "local-exec" {
    command = <<-EOT
      timeout 600 bash <<EOF
          until ssh ${local.ssh_args} -i /tmp/${random_string.identity_file.id} -o ConnectTimeout=2 -p ${var.ssh_port} root@${coalesce(self.ipv4_address, self.ipv6_address, try(one(self.network).ip, null))} test -e /etc/node-ready 2> /dev/null
          do
